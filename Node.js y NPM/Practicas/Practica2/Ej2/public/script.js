...
function renderUsers(usuarios) {
  ...
  if (usuarios.length > 0) {
    usuarios.forEach((usuario) => {
      ...
      texto.textContent = `ID: ${usuario.id} - ${usuario.nombre} ${usuario.apellidos} `;
...
async function añadirUsuario() {
  try {
    ...
    const fech_nac = document.querySelector("input[name='fech_nac']").value;
    ...
    if (!res.ok) {
        ...
        console.log(data); // Muestra errores
